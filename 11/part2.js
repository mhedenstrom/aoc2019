...
class Intcomp {
    ...
    mode(opcode, i) {
        ...
        if (m != ImmediateMode && m != PositionMode && m != RelativeMode)
            console.log("bad mode " + m)
        return m
    }
    ...
    decodeInstruction(pc) {
        ...
        // console.log(`${pc} ${instruction} - ${params}`)
...
// pc: ${pc}
// I: ${opcode}
// ${x.parameters.map(({position, mode}) => `${position} (mode ${mode})`).join('\n')}
//         `)
...
while(!c.halted) {
    ...
    paint(paintColor == 0 ? black : white)
    if (turn == 0) {
        turnCCW()
    } else {
        turnCW()
...
//     return Object.values(hull).reduce((sum, line) => {
//         return sum + Object.values(line).length
//     }, 0)
// }
